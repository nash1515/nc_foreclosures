"""Main enricher for Orange County Real Estate."""
...
from enrichments.orange_re.config import COUNTY_CODE
from enrichments.orange_re.scraper import search_by_address
from enrichments.orange_re.url_builder import build_property_url


logger = logging.getLogger(__name__)


class OrangeREEnricher(BaseEnricher):
    """Enricher for Orange County Real Estate URLs."""

    enrichment_type = 'orange_re'

    def enrich(self, case_id: int) -> EnrichmentResult:
        """
        Enrich a case with Orange County RE URL.

        Strategy:
            1. Parse address to get street number and name
            2. Search Orange County Spatialest portal via Playwright
        ...
        with get_session() as session:
            ...
            if case.county_code != COUNTY_CODE:
                return EnrichmentResult(
                    success=False,
                    error=f"Case {case.case_number} is not Orange County (code={case.county_code})"
                )

            logger.info(f"Enriching case {case.case_number} with Orange RE data")

            # Store case data for processing outside session
            case_number = case.case_number
            property_address = case.property_address

        # Require address for Orange
    ...
    def _enrich_by_address(self, case_id: int, case_number: str, property_address: str) -> EnrichmentResult:
        ...
        logger.debug(f"Searching Orange for: {street_number} {direction or ''} {street_name}")
    ...
    def _set_enrichment_fields(
        ...
        """Set Orange RE specific fields."""
        enrichment.orange_re_url = url
        enrichment.orange_re_parcel_id = account_id
        enrichment.orange_re_error = error
        enrichment.orange_re_enriched_at = datetime.now() if url else None
