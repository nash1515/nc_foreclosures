"""Main enricher for Lee County Real Estate."""
...
from enrichments.lee_re.config import COUNTY_CODE
from enrichments.lee_re.scraper import search_by_address
from enrichments.lee_re.url_builder import build_property_url


logger = logging.getLogger(__name__)


class LeeREEnricher(BaseEnricher):
    """Enricher for Lee County Real Estate URLs."""

    enrichment_type = 'lee_re'

    def enrich(self, case_id: int) -> EnrichmentResult:
        """
        Enrich a case with Lee County RE URL.

        Strategy:
            1. Parse address to get street number and name
            2. Search Lee County portal via Playwright
            3. If single match, capture the property URL
        ...
        with get_session() as session:
            ...
            if case.county_code != COUNTY_CODE:
                return EnrichmentResult(
                    success=False,
                    error=f"Case {case.case_number} is not Lee County (code={case.county_code})"
                )

            logger.info(f"Enriching case {case.case_number} with Lee RE data")

            # Store case data for processing outside session
            case_number = case.case_number
            property_address = case.property_address

        # Require address for Lee
    ...
    def _enrich_by_address(self, case_id: int, case_number: str, property_address: str) -> EnrichmentResult:
        ...
        direction = parsed.get('prefix')  # Directional prefix (N, S, E, W) goes to dropdown

        logger.debug(f"Searching Lee for: {street_number} {direction or ''} {street_name}")

        # Search via Playwright - pass direction separately for dropdown selection
        result = search_by_address(street_number, street_name, direction)

        if result.success and result.account_id:
            # Success - single match
            url = result.url or build_property_url(result.account_id)
            self._save_success(case_id, url, result.account_id)
            return EnrichmentResult(success=True, url=url, account_id=result.account_id)
    ...
    def _set_enrichment_fields(
        ...
        """Set Lee RE specific fields."""
        enrichment.lee_re_url = url
        enrichment.lee_re_account_id = account_id
        enrichment.lee_re_error = error
        enrichment.lee_re_enriched_at = datetime.now() if url else None
